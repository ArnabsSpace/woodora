...
export default function BannerSlider() {
  const slide = sliderData[0]; // Start with one
            ...
            <div className="BannerInnerSlide w-full flex flex-col-reverse md:flex-row items-center justify-between px-6 gap-8 relative z-2">
                ...
                <div className="flex-1 relative z-10 text-center sm:text-left">
                    <p className="text-red-600 font-bold text-lg">
                        {slide.price} <span className="line-through text-gray-500">{slide.oldPrice}</span>
                    </p>
                    ...
                    <button className="bg-primary text-white px-6 py-2 rounded-full hover:bg-primary-dark transition">Explore Now</button>
                </div>
                    ...
                    <div className="offerCircle rounded-full bg-red absolute z-1 w-40 h-40 p-1 flex items-center justify-center left-1/4">
                        <div className="offerCircleInner rounded-full w-full h-full bg-red border border-white p-1">
                            <p className="flex items-center flex-col h-full w-full justify-center text-white">
                                <span className="text-lg font-semibold">up to</span> <span className="text-5xl font-bold">50%</span> <span className="text-sm">on Selected Item</span>
                            </p>
                        </div>
                    </div>
                    <img src={slide.image} alt="Chair" className="mx-auto max-w-2xl w-full object-contain" />
                </div>
            </div>
